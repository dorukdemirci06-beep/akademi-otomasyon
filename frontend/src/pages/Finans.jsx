...
import { Wallet, PlusCircle, CheckCircle, CreditCard, ArrowDownRight, ArrowUpDown, 
 Calendar, BookOpen, AlertTriangle, ChevronRight, ChevronDown, Info, Clock, Trash2, X, FileText, Check, Edit, RotateCcw, Search, Sparkles, MessageCircle } from 'lucide-react';
...
const Finans = () => {
 ...
 {/* Toast Notification */}
 {toastMessage && (
 <div className="neo-card fixed bottom-6 right-6 z-[100] px-5 py-3.5 rounded-2xl backdrop-blur-md text-slate-900 dark:text-slate-100 -slate-600/80 flex items-center gap-3 text-sm font-semibold animate-in fade-in slide-in-from-bottom-4 duration-300 max-w-md">
 ...
 <div className="neo-card rounded-2xl -slate-200 p-6 space-y-4 text-slate-800 dark:text-slate-100">
 ...
 className="w-full bg-transparent text-xs font-bold text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none neo-input"
 />
 ) : selectedFormOgrenci ? (
 <span className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate">
 {selectedFormOgrenci.isim} {selectedFormOgrenci.soyisim} <span className="text-[#2eb82e] font-semibold">(₺{formatTL(selectedFormOgrenci.bakiye)})</span>
 </span>
 ) : (
 ...
 onChange={() => {}}
 className="opacity-0 absolute bottom-0 left-0 w-full h-0 pointer-events-none neo-input" 
 />
 ...
 <span className="text-[#2eb82e] px-2 py-0.5 rounded font-mono border shrink-0 ml-2">
 ₺{formatTL(o.bakiye)}
 </span>
 </div>
 );
 ...
 {taksitTarihleri.map((tarihVal, idx) => (
  ...
  <span className="text-xs font-bold text-slate-400 w-16 text-left shrink-0 whitespace-nowrap">
  {idx + 1}. Taksit:
  </span>
 <CustomDatePicker
 value={tarihVal}
 ...
 />
  {idx === 0 && (
  <span className="text-[10px] px-1.5 py-0.5 rounded font-bold bg-[#2eb82e] hover:bg-[#269926] transition-colors text-white border-transparent shadow-sm shrink-0 whitespace-nowrap">
  Bugün Alınacak
  </span>
  )}
 </div>
 ...
 <ChevronRight className="w-3.5 h-3.5" />
  ...
  {loading ? (
    ...
    <div className="text-center py-8 text-slate-400">Öğrenci bulunamadı.</div>
  ) : (
    sortedOgrenciler.map((o) => {
      const bakiye = o.bakiye || 0;
      const isOverdue = o.gecikmis_odeme_var_mi;
      ...
      return (
        <div
          key={o.id}
          onClick={() => setSelectedOgrenci(o)}
          className={`neo-card p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-3 shadow-sm bg-white dark:bg-[#15181e] cursor-pointer transition ${
            isOverdue ? '!border-l-4 !border-l-rose-500' : ''
          }`}
        >
          <div className="flex justify-between items-start border-b pb-2 border-slate-100 dark:border-slate-800/60">
            <div className="flex flex-col">
              <span className="font-bold text-slate-800 dark:text-slate-100 text-sm flex items-center gap-1.5">
                {o.isim} {o.soyisim}
                {isOverdue && (
                  <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded-full flex items-center gap-1 -transparent animate-pulse bg-rose-600 text-white shadow-sm">
          ...
          <div className="text-xs text-slate-600 dark:text-slate-300 space-y-2.5">
            <div className="flex justify-between items-center">
              <span className="font-semibold text-slate-500">Son Ödeme:</span>
              <span className="font-bold text-slate-800 dark:text-slate-200">
                {o.son_odeme_tarihi ? new Date(o.son_odeme_tarihi).toLocaleDateString('tr-TR') : '-'}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="font-semibold text-slate-500">Durum:</span>
              {isOverdue ? (
                <span className="-transparent text-[11px] px-2 py-0.5 rounded font-extrabold inline-flex items-center gap-1 bg-rose-600 text-white shadow-sm">
                  <AlertTriangle className="w-3 h-3" /> Gecikti
                </span>
              ) : bakiye > 0 ? (
                <span className="text-[#2eb82e] -transparent text-[11px] px-2 py-0.5 rounded font-extrabold inline-flex items-center gap-1 bg-[#2eb82e] text-white shadow-sm">
                  <CheckCircle className="w-3 h-3" /> Tahsil Edildi
                </span>
              ) : (
                <span className="-transparent text-[11px] px-2 py-0.5 rounded font-extrabold inline-flex items-center gap-1 bg-[#0284c7] text-white shadow-sm">
                  <ArrowDownRight className="w-3 h-3" /> Bekliyor
                </span>
              )}
            </div>
          </div>
          
          <div className="pt-1 border-t border-slate-100 dark:border-slate-800/60 mt-2">
            <button 
              onClick={(e) => {
                e.stopPropagation();
                setSelectedOgrenci(o);
              }}
              className="w-full mt-2 py-2 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full border border-slate-200 dark:border-slate-700/50 text-xs font-bold transition flex items-center justify-center gap-1.5"
            >
              <span>Detay & Taksitler</span>
              <ChevronRight className="w-3.5 h-3.5" />
            </button>
 ...
 <strong>{selectedOgrenci.isim} {selectedOgrenci.soyisim}</strong>
 </p>
 </div>
 <button
 onClick={() => {
 ...
 <div className="text-center py-10 neo-card space-y-2">
 ...
 <div className="neo-card -slate-300 rounded-2xl w-full max-w-md p-5 space-y-4 animate-in fade-in zoom-in duration-150">
