...
const HaftalikDersCizelgesi = ({
 ...
 className={`group/card relative cursor-pointer transition-all duration-300 transform hover:scale-[1.02] flex-1 min-w-0 ${
 isExpanded ? 'p-3' : 'p-2'
 } ${cardStyle} ${
 isSelected ? 'ring-2 ring-emerald-500 ring-offset-1 ring-offset-slate-100 dark:ring-offset-[#1b1f27]' : ''
 }`}
 >
 {!readonly && (
 <button
 ...
 <div className="mt-1.5 flex justify-between items-center text-[10px] font-semibold opacity-80 pt-1 border-t dark:border-white/10">
 <span className={isExpanded ? 'whitespace-normal font-bold' : 'truncate'}>{ders.sinif_adi}</span>
 {isSelected && (
 <span className="bg-emerald-500 text-white dark:text-slate-950 font-black px-1.5 py-0.5 rounded text-[8px] shrink-0 ml-1">
 SEÇİLİ
 </span>
 )}
 </div>
