...
const CustomDatePicker = ({ 
  ...
  return (
    <div className={`relative w-full ${className}`} ref={containerRef}>
      ...
      {/* 2X LARGER POPUP CALENDAR MODAL */}
      {isOpen && (
        <div 
          className={`absolute top-full mt-2 z-50 bg-slate-900/95 backdrop-blur-xl border border-slate-700/90 rounded-2xl shadow-2xl p-5 w-[380px] sm:w-[420px] animate-scale-in text-slate-100 ${
        ...
        >
          ...
          {/* Days Grid (2x Larger Cells) */}
          <div className="grid grid-cols-7 gap-1.5 text-center">
            {/* Prev month days */}
            {prevMonthDays.map((d, idx) => (
              <div
                key={`prev-${idx}`}
                className="h-11 sm:h-12 flex items-center justify-center text-xs font-semibold text-slate-600 opacity-40 select-none"
              >
                {d}
              </div>
            ))}
            ...
            {currentMonthDays.map((d) => {
              ...
              return (
                <button
                  ...
                  className={`h-11 sm:h-12 rounded-xl font-bold text-sm sm:text-base flex items-center justify-center relative transition transform hover:scale-105 cursor-pointer ${
                    selected
                      ? 'bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-950/60 ring-2 ring-emerald-400 font-extrabold'
                      : today
                      ? 'bg-slate-800 text-emerald-300 border border-emerald-500/80 font-black'
                      : 'bg-slate-800/60 hover:bg-emerald-600/30 hover:border-emerald-500/60 text-slate-200 border border-slate-800'
                  }`}
                >
                  <span>{d}</span>
                  {today && !selected && (
          ...
          {/* Footer Bar */}
          <div className="flex justify-between items-center border-t border-slate-800 pt-3 mt-4 text-xs font-bold">
            <button
              type="button"
              onClick={handleToday}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-950/80 hover:bg-emerald-900 text-emerald-300 border border-emerald-700/60 rounded-lg transition cursor-pointer"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-lg transition cursor-pointer"
            >
