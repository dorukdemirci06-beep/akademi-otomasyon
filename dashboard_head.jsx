...
const GUNLER = ['Pazartesi', 'Sal─▒', '├çar┼şamba', 'Per┼şembe', 'Cuma', 'Cumartesi', 'Pazar'];
const RENK_OPTIONS = [
  { label: '─░ndigo', value: 'indigo', bg: 'bg-indigo-100/90 border-indigo-300 text-indigo-950 dark:bg-indigo-950/80 dark:border-indigo-700/80 dark:text-indigo-200 shadow-sm' },
  { label: 'Z├╝mr├╝t', value: 'emerald', bg: 'bg-emerald-100/90 border-emerald-300 text-emerald-950 dark:bg-emerald-950/80 dark:border-emerald-700/80 dark:text-emerald-200 shadow-sm' },
  ...
  { label: 'G├╝l', value: 'rose', bg: 'bg-rose-100/90 border-rose-300 text-rose-950 dark:bg-rose-950/80 dark:border-rose-700/80 dark:text-rose-200 shadow-sm' },
  { label: 'Ye┼şil', value: 'green', bg: 'bg-green-100/90 border-green-300 text-green-950 dark:bg-emerald-900/70 dark:border-emerald-600/80 dark:text-emerald-200 shadow-sm' },
  { label: 'Sar─▒', value: 'yellow', bg: 'bg-yellow-100/90 border-yellow-300 text-yellow-950 dark:bg-yellow-950/80 dark:border-yellow-600/80 dark:text-yellow-200 shadow-sm' },
  { label: 'K─▒rm─▒z─▒', value: 'red', bg: 'bg-red-100/90 border-red-300 text-red-950 dark:bg-red-950/80 dark:border-red-600/80 dark:text-red-200 shadow-sm' }
];
...
const Dashboard = () => {
  ...
  const isAdmin = roleLower.includes('y├Ânetici') || roleLower.includes('yonetici') || roleLower.includes('admin') || roleLower.includes('super') || roleLower.includes('s├╝per');
  ...
  // S─▒n─▒f Ders Saati Ata/D├╝zenle Modal State
  ...
  // S─▒n─▒f ├û─şrencileri Detay Modal State
  ...
  const loadDashboardData = async () => {
    try {
      ...
      console.error('Dashboard veri y├╝kleme hatas─▒:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchSiniflarModal = async () => {
    try {
      ...
      console.error('S─▒n─▒flar y├╝klenemedi:', err);
    } finally {
  ...
  const getRegisteredTeachers = () => {
    try {
      ...
      return saved ? JSON.parse(saved) : [
        { id: 1, isim: 'Ahmet Y─▒lmaz', brans: 'Piyano & Solfej' },
        { id: 2, isim: 'Elif Kaya', brans: 'Keman & M├╝zik Teorisi' },
        { id: 3, isim: 'Caner ├ûzt├╝rk', brans: 'Dans & Koreografi' }
      ];
    } catch {
      return [
        { id: 1, isim: 'Ahmet Y─▒lmaz', brans: 'Piyano & Solfej' },
  ...
  const handleCreateSinif = async (e) => {
    ...
    try {
      ...
      showToast(`"${yeniSinifAdi.trim()}" s─▒n─▒f─▒ ${teacherName ? `(${teacherName} ├Â─şretmen atamas─▒ ile) ` : ''}ba┼şar─▒yla olu┼şturuldu!`);
    } catch (err) {
      showToast('Hata: S─▒n─▒f olu┼şturulamad─▒.', 'error');
    }
  };

  const handleDeleteSinif = (id, name) => {
    openConfirm({
      title: 'S─▒n─▒f Silme Onay─▒',
      message: `"${name}" s─▒n─▒f─▒n─▒ ve t├╝m ili┼şkili kay─▒tlar─▒ silmek istedi─şinize emin misiniz?`,
      type: 'danger',
      confirmText: 'S─▒n─▒f─▒ Sil',
      onConfirm: async () => {
        try {
          await deleteSinif(id);
          showToast(`"${name}" s─▒n─▒f─▒ ba┼şar─▒yla silindi.`);
          await fetchSiniflarModal();
        } catch (err) {
          console.error('S─▒n─▒f silinemedi:', err);
          showToast('S─▒n─▒f silinirken bir hata olu┼ştu.', 'error');
        }
      }
    });
  };
  ...
  const handleAddScheduleSlot = async (e) => {
    ...
    try {
      ...
      console.error('Ders saati atanamad─▒:', err);
    }
  };

  const handleDeleteScheduleSlot = (slotId) => {
    openConfirm({
      title: 'Ders Saati Silme Onay─▒',
      message: 'Bu ders saatini silmek istedi─şinize emin misiniz?',
      type: 'danger',
  ...
  const openSinifDetayModal = async (sinif) => {
    ...
    try {
      ...
      console.error('S─▒n─▒f ├Â─şrencileri y├╝klenemedi:', err);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Ho┼ş Geldiniz ­şæï</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Kurum ├Â─şrenci, yoklama ve finans s├╝re├ğlerinizi buradan y├Ânetin.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        >
          <div className="flex justify-between items-center">
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider group-hover:text-[#2eb82e] transition">TOPLAM ├û─ŞRENC─░</p>
          ...
          <div className="mt-4 flex items-center text-xs text-[#2eb82e] font-semibold gap-1">
            ...
            <span>Kay─▒tl─▒ ├û─şrenci Listesi ÔåÆ</span>
          </div>
        </Link>
        ...
        <div 
          ...
          title="T─▒klayarak s─▒n─▒flar─▒ g├Âr├╝nt├╝leyin ve y├Ânetin"
        >
          <div className="flex justify-between items-center">
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider group-hover:text-sky-500 transition">AKT─░F SINIF & BRAN┼Ş</p>
          ...
          <div className="mt-4 flex items-center text-xs text-sky-500 font-semibold gap-1">
            <TrendingUp className="w-4 h-4 shrink-0" />
            <span>S─▒n─▒flar─▒ G├Âr├╝nt├╝le & Y├Ânet ÔåÆ</span>
          </div>
        </div>

        {/* ├ûn Kay─▒t Adaylar Card */}
        <Link 
          to="/on-kayit" 
        ...
        >
          <div className="flex justify-between items-center">
            <div>
              <p className="text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider group-hover:text-emerald-500 transition">├ûN KAYIT ADAY L─░STES─░</p>
          ...
          <div className="mt-4 flex items-center text-xs text-emerald-600 dark:text-emerald-400 font-semibold gap-1">
            <TrendingUp className="w-4 h-4 shrink-0" />
            <span>Aday Listesini Y├Ânet ÔåÆ</span>
          </div>
        </Link>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/80 shadow-md p-6">
        <div className="flex justify-between items-center mb-5">
          <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <span>Son Kaydolan ├û─şrenciler</span>
          </h2>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider bg-slate-50 dark:bg-slate-900/50">
                ...
                <th className="py-3 px-4 whitespace-nowrap">├û─ŞRENC─░ ADI SOYADI</th>
                <th className="py-3 px-4 whitespace-nowrap">TELEFON</th>
                <th className="py-3 px-4 min-w-[220px]">ANNE / BABA VEL─░</th>
                {isAdmin && <th className="py-3 px-4 text-right whitespace-nowrap">BAK─░YE</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700/60 text-sm">
              {loading ? (
                <tr>
                  <td colSpan={isAdmin ? 5 : 4} className="text-center py-6 text-slate-400">Y├╝kleniyor...</td>
                </tr>
              ) : recentOgrenciler.length === 0 ? (
                <tr>
                  <td colSpan={isAdmin ? 5 : 4} className="text-center py-6 text-slate-400">Hen├╝z ├Â─şrenci kayd─▒ bulunamad─▒.</td>
                </tr>
              ) : (
                recentOgrenciler.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition">
                    ...
                    {isAdmin && <td className="py-3.5 px-4 font-bold text-[#2eb82e] text-right whitespace-nowrap">Ôé║{formatTL(o.bakiye)}</td>}
                  </tr>
                ))
              )}
            </tbody>
      ...
      {/* ================= MODAL: Mevcut S─▒n─▒flar & Kontenjan Y├Ânetimi ================= */}
      {showSiniflarModal && (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-5 text-slate-800 dark:text-slate-100">
            ...
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-3 sticky top-0 z-10 bg-white dark:bg-slate-800 pt-1">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                ...
                <span>Mevcut S─▒n─▒flar & Bran┼şlar</span>
              </h3>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <th className="py-3 px-4">SINIF / BRAN┼Ş ADI</th>
                    <th className="py-3 px-4">T├£M ATANAN DERS SAATLER─░</th>
                    <th className="py-3 px-4">KAYITLI ├û─ŞRENC─░</th>
                    <th className="py-3 px-4 text-right">─░┼ŞLEMLER</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700/60 text-sm">
                  {siniflarLoading ? (
                    <tr>
                      <td colSpan="5" className="text-center py-6 text-slate-400">S─▒n─▒flar y├╝kleniyor...</td>
                    </tr>
                  ) : siniflarList.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="text-center py-6 text-slate-400">Hen├╝z kay─▒tl─▒ s─▒n─▒f yok.</td>
                    </tr>
                  ) : (
                    siniflarList.map((s) => (
                      <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition">
                        ...
                        <td className="py-3.5 px-4">
                          {!s.ders_programi || s.ders_programi.length === 0 ? (
                            <span className="text-xs text-slate-400 dark:text-slate-500 italic">Hen├╝z saat atanmad─▒</span>
                          ) : (
                        ...
                        <td className="py-3.5 px-4 text-right space-x-2 whitespace-nowrap">
                          <button
                            ...
                            title="Ders G├╝n/Saat Ata & Y├Ânet"
                          >
                          ...
                          <button
                            ...
                            title="S─▒n─▒ftaki ├û─şrencileri G├Âr"
                          >
                            <Eye className="w-3.5 h-3.5 text-sky-500" />
                            <span>├û─şrenciler</span>
                          </button>
                          <button
      ...
      {/* ================= MODAL: Yeni S─▒n─▒f Ekle Sub-Modal ================= */}
      {showEkleModal && (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-4 text-slate-800 dark:text-slate-100">
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-3">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                ...
                <span>Yeni S─▒n─▒f / Bran┼ş Olu┼ştur</span>
              </h3>
            ...
            <form onSubmit={handleCreateSinif} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">S─▒n─▒f / Bran┼ş Ad─▒ *</label>
                <input
                  type="text"
                  required
                  placeholder="├ûrn: Piyano 101, Dans A Grubu"
                  value={yeniSinifAdi}
              ...
              <div>
                ...
                >
                  <option value="">-- ├û─şretmen Se├ğin --</option>
              ...
              {/* ─░ste─şe ba─şl─▒ Ders Program─▒ Atama Checkbox */}
              <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                <label className="flex items-center gap-2 text-xs font-bold text-sky-600 dark:text-sky-300 cursor-pointer">
                  ...
                  />
                  <span>Ders Program─▒na G├╝n & Saat Atans─▒n m─▒?</span>
                </label>
              </div>
              ...
              {addSchedule && (
                <div className="space-y-3 p-3 bg-slate-50 dark:bg-slate-900/80 rounded-xl border border-sky-200 dark:border-sky-700/60 text-xs">
                  <div>
                    <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">G├╝n Se├ğiniz</label>
                    <select
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Ba┼şlang─▒├ğ Saati</label>
                      <input
                        type="time"
                    ...
                    <div>
                      <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Biti┼ş Saati</label>
                      <input
                        type="time"
      ...
      {/* ================= MODAL: S─▒n─▒f Ders Saati Ata & Y├Ânet ================= */}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-5 text-slate-800 dark:text-slate-100">
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-3">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                ...
                <span>"{selectedSinifSchedule.sinif_adi}" ─░├ğin Ders Saati Ata</span>
              </h3>
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Mevcut Atanm─▒┼ş Saatler</h4>
              {!selectedSinifSchedule.ders_programi || selectedSinifSchedule.ders_programi.length === 0 ? (
                <p className="text-xs text-slate-400 dark:text-slate-500 italic py-2">Bu s─▒n─▒fa hen├╝z bir ders g├╝n├╝/saati atanmad─▒.</p>
              ) : (
            ...
            <form onSubmit={handleAddScheduleSlot} className="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">G├╝n</label>
                  <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Ba┼şlang─▒├ğ Saati</label>
                  <input
                    type="time"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Biti┼ş Saati</label>
                  <input
                    type="time"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">E─şitmen / ├û─şretmen (Opsiyonel)</label>
                <input
                  type="text"
                  placeholder="├ûrn: Ay┼şe ├û─şretmen"
      ...
      {/* ================= MODAL: S─▒n─▒f ├û─şrencileri Detay Listesi ================= */}
      {selectedSinifDetay && (
        <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto p-6 shadow-2xl border border-slate-200 dark:border-slate-700 space-y-4 text-slate-800 dark:text-slate-100">
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 pb-3">
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <BookOpen className="w-5 h-5 text-sky-500" />
                <span>"{selectedSinifDetay.sinif_adi}" S─▒n─▒f─▒ndaki ├û─şrenciler</span>
              </h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 dark:text-slate-400 tracking-wider bg-slate-50 dark:bg-slate-900/50">
                    <th className="py-3 px-4">├û─ŞRENC─░ ID / TC</th>
                    <th className="py-3 px-4">ADI SOYADI</th>
                    <th className="py-3 px-4">─░LET─░┼Ş─░M</th>
                    <th className="py-3 px-4">VEL─░ B─░LG─░S─░</th>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700/60 text-sm">
                  {detayLoading ? (
                    <tr>
                      <td colSpan="5" className="text-center py-6 text-slate-400">├û─şrenciler y├╝kleniyor...</td>
                    </tr>
                  ) : sinifOgrencileriList.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="text-center py-6 text-slate-400">Bu s─▒n─▒fa hen├╝z kay─▒tl─▒ ├Â─şrenci yok.</td>
                    </tr>
                  ) : (
